...
class TransactionsTabPageCompany extends StatefulWidget {
  static const route = "/transactions_tab_page_company";
  final StationWithTransactionsTotal company;

  const TransactionsTabPageCompany({Key? key, required this.company}) : super(key: key);

  @override
  State<TransactionsTabPageCompany> createState() => _TransactionsTabPageCompanyState();
}
...
class _TransactionsTabPageCompanyState extends State<TransactionsTabPageCompany> {
  @override
    ...
    return BlocListener<StationBloc, StationState>(
        listener: (context, state) async {
          if (state is InitStationState) {
            ...
            currentPage = 1;
            loadMore = false;
            bloc.add(GetMonthlyStationPaymentTransactionsByCompanyIdEvent(1, widget.company.id!));
      ...
      notificationsController.stream.listen((notificationBody) {
        if (mounted) {
          ...
          if (!widget.bloc.isClosed)
            widget.bloc.add(GetMonthlyStationPaymentTransactionsByCompanyIdEvent(1, widget.company.id!));
        }
      });
    }
          ...
          InkWell(
              onTap: () async {
                ...
                if (monthlyPaymentTransactionList.length > 0)
                  if (!widget.bloc.isClosed)
                    widget.bloc.add(GetMonthlyPaymentTransactionPDFByCompanyIdEvent(
                        monthlyPaymentTransactionList, widget.company.id!));
                  ...
                  child: CustomLoading(),
                    ...
                    builder: (context, constraints) => RefreshIndicator(
                        onRefresh: () async {
                          ...
                          if (!widget.bloc.isClosed)
                            widget.bloc.add(
                                GetMonthlyStationPaymentTransactionsByCompanyIdEvent(
                                    1, widget.company.id!));
                        },
                        color: Colors.white,
      ...
      if (hasNext) {
        if (controller.position.pixels >
          ...
          setState(() {
            ...
            if (!widget.bloc.isClosed)
              widget.bloc
                  .add(GetMonthlyStationPaymentTransactionsByCompanyIdEvent(currentPage, widget.company.id!));
          });
        }
      }
    }
